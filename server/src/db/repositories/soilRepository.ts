...
import { SoilProfile, RegionalSoilBaseline } from '../../models/Database';

export class SoilRepository {
  async getLatestProfile(fieldId: string): Promise<SoilProfile | null> {
    return queryOne<SoilProfile>(
  ...
  async getProfileHistory(fieldId: string): Promise<SoilProfile[]> {
    return query<SoilProfile>(
      `SELECT id, field_id, source, texture, organic_matter_pct::float,
              nitrogen_level, phosphorus_level, potassium_level,
              water_holding_capacity, ph::float, report_date::text,
              raw_document_url, summary_text, created_at::text
  ...
  async insertProfile(
    profile: Omit<SoilProfile, 'id' | 'created_at'>
  ): Promise<SoilProfile> {
    const row = await queryOne<SoilProfile>(
      `INSERT INTO soil_profiles
       ...
       RETURNING id, field_id, source, texture, organic_matter_pct::float,
                 ...
                 raw_document_url, summary_text, created_at::text`,
      [
        profile.field_id, profile.source, profile.texture,
        profile.organic_matter_pct, profile.nitrogen_level,
        profile.phosphorus_level, profile.potassium_level,
        profile.water_holding_capacity, profile.ph,
        profile.report_date, profile.raw_document_url ?? null,
  ...
  async getRegionalBaseline(regionId: string): Promise<RegionalSoilBaseline | null> {
    ...
    return {
      ...
      avg_npk: { n: row.avg_nitrogen, p: row.avg_phosphorus, k: row.avg_potassium },
      avg_ph: row.avg_ph,
