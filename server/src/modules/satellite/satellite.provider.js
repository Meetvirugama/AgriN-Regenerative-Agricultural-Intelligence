export class MockSatelliteProvider {
  generateMockSubregions() {
    const labels = [
      "Northeast",
      "Northwest",
      "Southeast",
  ...
  createMockTile(fieldId, date, options) {
    const subregions = this.generateMockSubregions();
    const ndviMean =
      subregions.reduce((sum, s) => sum + s.ndvi, 0) / subregions.length;
    return {
      ...
      cloudCoverPct:
        Math.random() > 0.8 ? Math.random() * 100 : Math.random() * 10, // Mostly clear, sometimes cloudy
      tileUrl: null,
  ...
  async fetchLatestTile(fieldBoundary, fieldId) {
    ...
    if (isCloudy) {
      return this.createMockTile(fieldId, date.toISOString(), {
        cloudCoverPct: 80 + Math.random() * 20,
      });
    }
  ...
  async getHistoricalTiles(fieldId, from, to) {
    const tiles = [];
