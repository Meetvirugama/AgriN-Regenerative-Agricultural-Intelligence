...
export interface IWeatherProvider {
  /**
   * Fetch weather forecast for the next 7 days for a given location.
   */
  getForecast(lat: number, lng: number, fieldId: string): Promise<FieldWeatherSnapshot[]>;
  
  /**
   * Fetch historical actuals for the last 30 days.
   */
  getHistory(lat: number, lng: number, fieldId: string): Promise<FieldWeatherSnapshot[]>;
}

export class MockWeatherProvider implements IWeatherProvider {
  public async getForecast(lat: number, lng: number, fieldId: string): Promise<FieldWeatherSnapshot[]> {
    const snapshots: FieldWeatherSnapshot[] = [];
  ...
  public async getHistory(lat: number, lng: number, fieldId: string): Promise<FieldWeatherSnapshot[]> {
    const snapshots: FieldWeatherSnapshot[] = [];
    const today = new Date();
