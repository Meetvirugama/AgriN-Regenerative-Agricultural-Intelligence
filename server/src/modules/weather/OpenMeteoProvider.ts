...
export class OpenMeteoProvider implements IWeatherProvider {
  private readonly defaultLat = 30.9;
  private readonly defaultLng = 75.8;
  private readonly timeout = 8000; // 8 seconds

  private async fetchWithTimeout(url: string): Promise<OpenMeteoResponse> {
  ...
  async getForecast(lat: number, lng: number, fieldId: string): Promise<FieldWeatherSnapshot[]> {
  ...
  async getHistory(lat: number, lng: number, fieldId: string): Promise<FieldWeatherSnapshot[]> {
    const useLat = lat || this.defaultLat;
    const useLng = lng || this.defaultLng;
