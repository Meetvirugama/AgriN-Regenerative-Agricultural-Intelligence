...
router.get("/", async (req, res, next) => {
  try {
    ...
    const alerts = result.map((row) => ({
      ...
      field: row.field_name || "All Fields",
      source: row.source,
 ...
 */
router.patch("/:alertId/read", validateUuidParam("alertId"), async (req, res, next) => {
  try {
    const farmerId = req.farmer?.sub;
 ...
 */
router.patch("/:alertId/resolve", validateUuidParam("alertId"), async (req, res, next) => {
  try {
    const farmerId = req.farmer?.sub;
    const { alertId } = req.params;
