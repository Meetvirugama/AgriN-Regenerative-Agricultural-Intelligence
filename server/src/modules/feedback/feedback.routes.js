  ...
  try {
    ...
    if (!field_id || !farmer_response) {
      res
        .status(400)
        .json({
          error: { message: "field_id and farmer_response are required" },
        });
      return;
    }
 ...
 */
router.post("/jobs/trigger-feedback-prompts", async (_req, res) => {
  // Phase 5: implement stale advisory detection and create feedback_events rows
  res.json({
    message:
      "Checked for stale advisories. Job not yet implemented — see Phase 5.",
  });
});
 ...
 */
router.post("/jobs/verify-outcomes", async (_req, res) => {
  // Phase 5: implement satellite change detection comparison
  res.json({
    message:
      "Satellite outcome verification: not yet implemented — see Phase 5.",
  });
});
