  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="font-bold text-sm tracking-wide">{currentLangName.split(' ')[0]}</span>
      </button>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-surface border-2 border-black rounded-xl shadow-xl z-50 overflow-hidden">
          <div className="p-3 bg-neutral/10 border-b border-black">
            <h4 className="text-xs font-black uppercase tracking-widest text-text-muted">Select Language</h4>
          </div>
          <div className="flex flex-col">
            {LANGUAGES.map((lang) => {
              ...
              return (
                ...
                >
                  <span className="font-bold text-lg">{lang.name.replace(' (Text Only)', '')}</span>
                  {isTextOnly && (
                    <span className="text-xs font-semibold text-warning mt-1">Audio features not yet supported</span>
                  )}
                </button>
              );
            })}
