...
export function DiseaseDiagnosisFlow({ fieldId, onClose }) {
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      const data = await diagnosisApi.diagnoseCrop(fieldId, base64String);
    ...
    return (
      <div className="flex items-center gap-1">
        <div className="flex gap-0.5">
          <div
            className={`w-3 h-3 rounded-full ${tier >= 1 ? "bg-primary" : "border-2 border-primary"}`}
          ></div>
          <div
            className={`w-3 h-3 rounded-full ${tier >= 2 ? "bg-primary" : "border-2 border-primary"}`}
          ></div>
          <div
            className={`w-3 h-3 rounded-full ${tier >= 3 ? "bg-primary" : "border-2 border-primary"}`}
          ></div>
        </div>
      ...
      <div className="flex flex-col min-h-[400px]">
            ...
            >
              {capturedImageUrl ? (
                ...
                <>
                  ...
                  <p className="text-primary font-bold text-sm text-center px-4">
                    Tap to add photo
                  </p>
                </>
              )}
        ...
        {step === "analyzing" && (
          <div className="flex flex-col items-center justify-center flex-1 py-12">
            <ScanSearch
              size={64}
              className="text-primary animate-bounce mb-6"
            />
            <h3 className="font-bold text-xl animate-pulse">
              Looking closely at your photo...
            </h3>
            <p className="text-text-muted mt-2 text-sm text-center">
              Checking field history and recent weather context.
            </p>
          </div>
        )}
        ...
        {step === "result" && result && (
          <div className="flex flex-col flex-1">
            <div className="mb-6 flex flex-col gap-2">
              <h4 className="uppercase tracking-widest text-text-muted text-xs font-bold">
                Diagnosis
              </h4>
          ...
          <div className="flex flex-col flex-1">
              ...
              {result.predicted_category === "unknown" ? (
                <div className="flex flex-col items-center text-center gap-4">
                  ...
                  <p className="text-text-muted">
                    The photo might be unclear, or this could be a rare issue.
                    We shouldn't guess when it comes to your crop.
                  </p>
                </div>
              ) : (
                <div className="flex flex-col items-center text-center gap-4">
                  ...
                  <p className="text-text-muted">
                    This looks like {result.predicted_label}. Because this can
                    spread quickly, we need to escalate this immediately.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-surface border-2 border-neutral rounded-xl p-6 mb-8 mt-4 text-center">
              <h3 className="font-bold text-lg mb-2">
                Let's get a human to look.
              </h3>
              <p className="text-sm text-text-muted mb-4">
                We'll send your photo and field history directly to your local
                extension officer.
              </p>
              ...
              <div className="flex items-start gap-3 text-left bg-neutral/10 p-3 rounded-lg mb-6 border border-neutral">
                ...
                <label
                  htmlFor="escalation-consent"
                  className="text-xs text-text-muted font-medium cursor-pointer leading-tight"
                >
                  By checking this box, I explicitly consent to sharing this
                  field's diagnosis history and recent satellite metrics with my
                ...
                onClick={async () => {
                  try {
                    ...
                    await escalationApi.triggerEscalation(
                      ...
                      "Layer07",
                      {
                        issue: result.predicted_label,
                        confidence: result.confidence,
                        consentVerified: true,
                      },
                    );
                    console.log(
                      "Escalation sent successfully! An expert will reach out soon.",
                    );
                    onClose();
                  } catch {
