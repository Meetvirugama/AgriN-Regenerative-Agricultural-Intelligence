...
export const SatelliteTrendChart = ({ timeline }) => {
  if (!timeline || timeline.length === 0) {
    return (
      <div className="p-4 text-center text-text-muted text-sm border border-border">
  ...
  const getX = (index) => (index / (timeline.length - 1)) * 100;
  const getY = (value) =>
    height - ((value - minVal) / (range || 1)) * height * 0.8 - height * 0.1;

  // Generate path
  const points = timeline
    .map((t, i) => `${getX(i)},${getY(t.ndviValue)}`)
    .join(" L ");
  ...
  return (
    <div className="w-full bg-background border border-border p-4">
      <div className="flex justify-between items-center mb-4">
        <h4 className="font-bold uppercase text-sm tracking-widest text-text-muted">
          Vegetation Trend (NDVI)
        </h4>
      </div>
      ...
      <div className="relative w-full h-[150px]">
        ...
        <div className="ml-8 w-[calc(100%-2rem)] h-full relative border-l border-b border-border/30">
          ...
          <div className="absolute -bottom-6 w-full flex justify-between text-[10px] text-text-muted">
            <span>
              {new Date(timeline[0].date).toLocaleDateString(undefined, {
                month: "short",
                day: "numeric",
              })}
            </span>
