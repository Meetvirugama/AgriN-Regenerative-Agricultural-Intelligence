...
export const SatelliteFieldMap: React.FC<SatelliteFieldMapProps> = ({ boundary, data, onAnomalyClick }) => {
  ...
  const center = React.useMemo(() => {
    if (!boundary || boundary.length === 0) return { lat: 20.5937, lng: 78.9629 };
    ...
    <div className="relative w-full h-[400px] border border-border bg-surface">
      {hasCloudCover && (
         <div className="absolute inset-0 z-10 bg-background/60 backdrop-blur-sm flex flex-col items-center justify-center text-text-main p-6 text-center">
            ...
            <h3 className="font-bold uppercase tracking-widest mb-2">Cloud Cover</h3>
            <p className="text-sm max-w-xs">Latest satellite pass was obstructed by clouds. Showing last known clear data below.</p>
         </div>
      )}
