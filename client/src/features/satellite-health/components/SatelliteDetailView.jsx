...
export const SatelliteDetailView = ({ fieldId, fieldBoundary, onClose }) => {
  ...
  if (loading) {
    return (
      <div className="p-8">
        <LoadingSkeleton message="Loading satellite data..." />
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-8">
        <ErrorState title="Satellite Data Failed" message={error} />
      </div>
    );
  }

  const activeAnomaly = data?.activeAnomalies.find(
    (a) => a.id === selectedAnomaly,
  );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
      <div className="sticky top-0 bg-background border-b border-border p-4 flex justify-between items-center z-20">
        ...
        <button
          onClick={onClose}
          className="p-2 border border-border hover:bg-surface"
        >
          <X size={24} />
        </button>
      </div>
        ...
        <section>
          <div className="mb-4">
            ...
            <p className="text-sm text-text-muted">
              {data?.latestTile
                ? `Pass from ${new Date(data.latestTile.captureDate).toLocaleDateString()}`
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-bold uppercase tracking-widest">
                    Stress Detected: {activeAnomaly.subregionLabel}
                  </h4>
                  <p className="text-sm text-text-muted">
                    Detected{" "}
                    {new Date(activeAnomaly.detectedDate).toLocaleDateString()}.
                    Vegetation index has dropped significantly vs recent passes.
                  </p>
                </div>
              </div>
              <Button
