...
export const SatelliteHealthCard = ({ data, loading, onClick }) => {
  if (loading) {
    return (
  ...
  // Format date
  const dateStr = new Date(latestTile.captureDate).toLocaleDateString(
    undefined,
    {
      month: "short",
      day: "numeric",
  ...
  const daysAgo = Math.floor(
    (new Date().getTime() - new Date(latestTile.captureDate).getTime()) /
      (1000 * 3600 * 24),
  );
  const timeText =
        ...
        <div>
          ...
          {latestTile.cloudCoverPct > 30 && (
            <p className="text-xs text-text-muted mt-2 flex items-center gap-1">
              <CloudRain size={12} /> Partial cloud cover (
              {Math.round(latestTile.cloudCoverPct)}%)
            </p>
          )}
        </div>
      </div>
