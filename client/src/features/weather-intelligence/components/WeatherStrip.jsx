...
export function WeatherStrip({ forecasts = [], flags = [], isLoading, onExpand }) {
  ...
  if (flags.length > 0) {
    ...
    if (topFlag.message) {
      ...
      if (topFlag.event_type === "rain_expected" && topFlag.severity !== "high")
        messageColor = "text-success";
    }
  }

  const getWeatherIcon = (snapshot) => {
    if (snapshot.rainfall_mm > 5)
      return <CloudRain size={24} className="text-primary" />;
    if (snapshot.humidity_pct > 80)
      return <Cloud size={24} className="text-text-muted" />;
  ...
  return (
    ...
    >
      ...
      {primaryMessage && (
        <div
          className={`text-sm font-bold flex items-center gap-2 ${messageColor} text-right`}
        >
          {primaryMessage}
          <TextToSpeechButton
