...
export function ClimateRiskWidget({ fieldId }) {
  ...
  useEffect(() => {
    const fetchRisk = async () => {
      try {
        setLoading(true);
        const result = await request(`fields/${fieldId}/climate-risk`);
        setData(result);
      } catch (err) {
  ...
  const getSeverityColors = (statusLevel) => {
    switch (statusLevel) {
      case "urgent":
  ...
  return (
    <Card className={cn("transition-all", getSeverityColors(status))}>
      <div className="flex items-start justify-between mb-4">
        ...
        <StatusBadge status={status}>{data.severity} Risk</StatusBadge>
      </div>

      <div className="bg-surface/60 rounded-lg p-4 text-sm font-medium leading-relaxed border border-border/50 text-text">
        <strong className="block text-text-muted text-xs uppercase tracking-wider mb-1">
          Recommended Action
        </strong>
