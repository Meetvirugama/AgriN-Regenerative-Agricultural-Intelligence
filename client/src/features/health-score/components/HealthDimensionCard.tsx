...
import { DimensionResult } from '../types/health-score.types';
...
export const HealthDimensionCard: React.FC<HealthDimensionCardProps> = ({ title, dimension, icon }) => {
        ...
        <div className="p-4 pt-0 border-t border-border/50 bg-surface text-sm animate-fade-in">
          <ul className="space-y-2 mt-4 list-disc list-inside">
            {dimension.basis.map((reason, idx) => (
              <li key={idx} className="text-text-main">{reason}</li>
            ))}
            {dimension.basis.length === 0 && (
              <li className="text-text-muted">No specific details available.</li>
            )}
          </ul>
        </div>
      )}
