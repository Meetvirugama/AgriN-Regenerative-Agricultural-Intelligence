...
export const Field: React.FC = () => {
  ...
  const { cropState, isLoading, error: initError, setCropState } = useActiveField();
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const [weatherResult, soilResult, promptsResult, timelineResult] = await Promise.allSettled([
        weatherApi.getForecast(fieldId as string),
        soilApi.getSoilProfile(fieldId as string),
        memoryApi.getPendingPrompts(fieldId as string),
        memoryApi.getTimeline(fieldId as string),
      ]);
