...
interface Props {
   ...
   * Defaults to "This section" for generic boundaries.
   */
  sectionName?: string;
  /**
   * If true, shows a compact inline error instead of a full card.
   * Use for small widgets where a full card would be disruptive.
   */
  compact?: boolean;
}
    ...
    if (compact) {
      return (
        <div className="flex items-center gap-2 p-3 bg-danger/5 border border-danger/20 text-danger text-sm rounded-lg">
          ...
          <span className="flex-1 font-medium">{sectionName} could not load.</span>
          <button
      ...
      <div className="p-5 bg-surface border border-neutral rounded-xl">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-sm text-text mb-1">{sectionName} unavailable</h3>
            <p className="text-xs text-text-muted leading-relaxed">
              This section could not be loaded. The rest of your field dashboard is unaffected.
            </p>
